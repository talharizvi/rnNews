...
const Sports=({navigation})=>{
    const[sportsData,setData]=useState([]) 
    ...
    useEffect(()=>{
        axios.get(`https://newsapi.org/v2/top-headlines?country=us&apiKey=ac659059bcc34c4b9c74dfa215ff2eb7&category=sports&pageSize=10&page=${page}`)
        .then(response=>{
    ...
    const loadNextNews=()=>{
        ...
        .then(response=>{
            ...
            setData([...sportsData,...response.data.articles])
        })
    }
